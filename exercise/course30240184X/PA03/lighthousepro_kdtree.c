...
int main(){
    int n, k, i;
    KdTreeType* ords, x, y, query[4], range[] = {0, 0, 10000000, 10000000};
    KdTree* kt = newKdTree();
    reset_io();
    scanf("%d", &n);
    n <<= 1;
    ords = (KdTreeType*)malloc(sizeof(KdTreeType)*n);
    for(i = 0; i < n; i += 2) {
        scanf("%d %d", ords + i, ords + i + 1);
        insertIntoKdTree(kt, ords + i, &compare);
    }
    scanf("%d", &k);
    for(i = 0; i < k; i++) {
        scanf("%d %d %d %d", query, query + 1, query + 2, query + 3);
        printf("%d\n", rangeSearch(kt, range, query, &compare, &visit) + rangeSearch(kt, query + 2, range + 2, &compare, &visit));
    }
    free(ords);
    freeKdTree(kt);
    return 0;
