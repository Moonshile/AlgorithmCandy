...
public:
    explicit GraphNode(string &n): name(n), val(INT_MAX), neighbors() {}
    
    void addNeighbor(const shared_ptr<GraphNode> node) {
        neighbors.push_back(node);
    }

    unsigned int countNeighbor() const {
    ...
    vector<vector<string>> shortestPathsToSource() const {
        vector<vector<const GraphNode*>> path1, path2, *cur = &path1, *next = &path2;
        int next_distance = val;
        if (val < INT_MAX) {
            path1.push_back(vector<const GraphNode*>{this});
...
class Solution {
private:
    vector<string> nextNames(const string &cur, const unordered_set<string> &dict) {
        vector<string> res;
        for (unsigned int i = 0; i < cur.size(); ++i) {
            string tmp(cur);
            for (char c = cur[i], j = 'a'; j <= 'z'; ++j) {
                if (j == c) {
                    continue;
                }
                tmp[i] = j;
...
public:
    vector<vector<string>> findLadders(string start, string end, unordered_set<string> &dict) {
        ...
        while (!q.empty()) {
            ...
            auto next_names = nextNames(cur, dict);
            for (string name: next_names) {
                nodes[cur].get()->addNeighbor(nodes[name]);
                if (!accessed[name] && !to_end) {
                    q.push(name);
                }
                to_end = to_end || name == end;
            }
            accessed[cur] = true;
        }
...
int main() {
    ...
    auto res = s.findLadders("sand", "acne", dict);
